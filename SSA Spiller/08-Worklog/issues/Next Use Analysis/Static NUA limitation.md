...
[[02-Components/MachineLaneSSAUpdater|Machine Lane SSA Updater]] insert PHI nodes on the reload parent block IDF and also creates new registers.
Since we run NUA statically before the spiller, **new SSA variables have no entry in the NUA map and, hence, considered dead**.

![[Pasted image 20251222122422.png]]
...
For that we change [[02-Components/MachineLaneSSAUpdater|Machine Lane SSA Updater]]  API method repairSSAForNewDef signature to accept a reference to vector of Machine operands which will be filed with the inserted PHIs result operands.
