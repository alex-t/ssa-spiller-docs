...
- Kills spilled live interval in the CFG subgraph dominated by the kill point [[Decisions#Design Change Prevent SSA Repair Disorder by Killing Spilled LiveIntervals in Dominated Region]]
...
- PHIs during SSA repair (via `MachineLaneSSAUpdater`).

Each reload/PHI creates a **new SSA variable** (new virtual register / new live interval).
We run Next Use Analysis before the spiller when these new register don't yet exist. As a result, they have no entries in the NUA map and analysis consider these new vregs **dead**, which can immediately select them for spilling again — yielding **no RP relief** and eventually an RP validation failure. [[Static NUA limitation]]
...
[[Static NUA limitation#Proposed solution]]
### Temporary workaround (implemented now)
Maintain a `VRegMaskPairSet` of vregs created by reload SSA repair and exclude them from the spiller’s Active set selection.

[[Static NUA limitation#Temporary workaround]]
To make this possible, `MachineLaneSSAUpdater::repairSSAForNewDef` was extended to report back the **def operands of inserted PHI results**:
...
- Spiller usage (AMDGPU):
  - `AMDGPUSSARegisterSpiller::repairSSAForReload` passes `PHIRegDefOps` and inserts them into `ReloadedRegs` as `VRegMaskPair` entries.

See also the design decision [[Decisions#Static Next Use Analysis limitation]]
