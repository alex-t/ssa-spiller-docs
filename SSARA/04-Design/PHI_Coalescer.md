...
### Related
- [SSA_RA_Coloring](SSA_RA_Coloring.md) — the phase that runs immediately before this one.
- [SSA_Register_Allocator_Impl](../02-Components/SSA_Register_Allocator_Impl.md) — `color()`, `lowerPHIs()`, `resolvePermutation()`.
- [SSA_Destruction](../02-Components/SSA_Destruction.md) — where fixed points turn into "no copy".
...
> ([SSA_RA_Coloring](SSA_RA_Coloring.md)) *before* the coalescer. The coalescer **never colors and
...
>    for." The coloring produced by `color()` (§[SSA_RA_Coloring](SSA_RA_Coloring.md)) is already a
