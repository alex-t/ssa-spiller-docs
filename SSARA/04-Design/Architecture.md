...
- In SSA destruction, an AGPR permutation cycle cannot use `V_SWAP`/XOR (no such
  ...
  [SSA_RA_Coloring](SSA_RA_Coloring.md#ssa-destruction-phi-lowering-permutation-resolution).
...
| **Reload placement** | [Reload_join_phi_coalescing](Reload_join_phi_coalescing.md) | Active (supersedes [Reload_optimizer](Reload_optimizer.md)) |
| **SSA Register Allocator** | [SSA_RA_Coloring](SSA_RA_Coloring.md) | Active |
| **SimplifyUndefPHI** | [PHI_Coalescer](PHI_Coalescer.md) (§ current reality) | Active (undef-PHI slice; runs before spiller) |
| **MachineLaneSSAUpdater** | [MachineLaneSSAUpdater](MachineLaneSSAUpdater.md) | Active |
| **Per-class RP tracker** | [GCNUpwardRPTracker_PerClassRP](GCNUpwardRPTracker_PerClassRP.md) | Proposed |
| **Fragmentation-aware spiller** | [Spiller_Redesign](Spiller_Redesign.md) | Proposed |
| **Next Use Analysis** | [NextUseAnalysis](NextUseAnalysis.md) · [Persistent_Map_for_NUA](Persistent_Map_for_NUA.md) | Active |
...
[Decisions](Decisions.md#store-at-definition).
...
The spiller keeps SSA form by using [MachineLaneSSAUpdater](MachineLaneSSAUpdater.md) to repair SSA
...
|---------|-------------|----------|
| PHI coalescer — greedy affinity (Option B + sub-reg hints) | ✅ Committed to `ssara` (`3deb087e`); `pickFreePhysReg` consults `collectPhiHints`. Greedy color choice, not yet recoloring. See [PHI_Coalescer](PHI_Coalescer.md#101-status-2026-07-14) | — |
| Undef-PHI simplifier + PHI-copy metric | ✅ Committed to `ssara` (`80fc7d8d`, `da78e671`); `AMDGPUSimplifyUndefPHI` runs before the spiller | — |
| PHI coalescer — real recoloring (paper §4.3, Option A) | Recolor PHI operands to reduce copies (design: [PHI_Coalescer](PHI_Coalescer.md)); durable fix for cross-call [physreg-exhaustion](SSA_RA_Coloring.md#cross-call-color-constraint) — 99.8% of residual copies feasible | High |
| Per-class RP / feasibility gate | [GCNUpwardRPTracker_PerClassRP](GCNUpwardRPTracker_PerClassRP.md) + [Spiller_Redesign](Spiller_Redesign.md) fragmentation-aware spilling & greedy fallback | High |
...
- **Pruned-IDF / reload-optimizer / NCD-hoisting** — removed; superseded by
  [Reload_join_phi_coalescing](Reload_join_phi_coalescing.md) (cut-LI dominance-ordered reconstruction).
- **LR splitter** — removed; width-descending coloring reuses freed slots
  (see [SSA_RA_Coloring](SSA_RA_Coloring.md#d3-splitter-removal)).
- **Interval-killing before SSA repair** — superseded; see [Decisions](Decisions.md).
