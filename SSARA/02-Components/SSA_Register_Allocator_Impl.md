...
> (the [SSA_Spiller](SSA_Spiller.md) runs as a separate pass beforehand). If coloring cannot
> find a free physreg it asserts `Failed to find free physreg` rather than
> spilling on demand.

## Spilling Model

Spilling is handled by a **separate pre-pass**, the [SSA_Spiller](SSA_Spiller.md), which runs
...
[MachineLaneSSAUpdater](../04-Design/MachineLaneSSAUpdater.md)) is documented under [SSA_Spiller](SSA_Spiller.md).
