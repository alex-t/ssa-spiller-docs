...
[[Next_Use_Analysis]]`::getNextUseDistance`.
...
[[MachineLaneSSAUpdater]]`::repairSSAForNewDef` (one call per reload redef), so
...
✅ Belady/MIN victim selection via [[Next_Use_Analysis]]  
...
✅ Inline SSA repair via [[MachineLaneSSAUpdater]] (spiller returns SSA)
